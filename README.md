...
```
dbx execute --cluster-name=<name of interactive cluster> --job=martech_pipelines-sample-integration-test
```

For a test on an automated job cluster, deploy the job files and then launch:
```
dbx deploy --jobs=martech_pipelines-sample-integration-test --files-only
dbx launch --job=martech_pipelines-sample-integration-test --as-run-submit --trace
```
...
- To trigger the release pipeline, get the current version from the `martech_pipelines/__init__.py` file and tag the current code version:
```
